...
const RecentUser: React.FC<RecentUserProps> = ({ users }) => {
  return (
    <div
  className="bg-gradient-to-b from-white/15 via-blue-8z0/15 to-blue-800/0 p-6 rounded-lg space-y-6 border-2 border-white/20 h-68"
      style={{
        ...
        <div key={index} className="flex items-center justify-between space-x-4 py-3">
          <div className="flex items-center space-x-3">
            ...
            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center relative">
              <span className={`w-3 h-3 rounded-full absolute top-0 left-0 ${user.status === 'online' ? 'bg-green-500' : 'bg-gray-400'}`} />
            </div>
            {/* User Info */}
            <div className="text-white">
              <div className="font-medium">{user.name}</div>
              <div className="text-sm text-gray-400">{user.status === 'online' ? 'New hire' : 'Developer'}</div>
            </div>
          </div>
