...
const UserDropdown = ({ username, role }: UserDropdownProps) => {
  ...
  useEffect(() => {
    if (dropdownOpen && btnRef.current && dropdownRef.current) {
      popperInstance.current = new Popper(btnRef.current, dropdownRef.current, {
  ...
  return (
    <div className="relative">
      ...
      <button
        ref={btnRef}
        onClick={toggleDropdown}
        className="group flex items-center gap-3 rounded-full bg-white/10 border border-white/15 px-3 h-12 hover:bg-white/15 transition shrink-0"
        aria-label="User menu"
      >
        {/* Avatar */}
        <span className="relative inline-grid place-items-center">
      ...
      <div
        ...
        } z-50 w-44 rounded-xl border border-white/10 bg-[#101B31]/95 backdrop-blur-lg shadow-xl`}
      >
        <ul className="p-2">
          {/* Settings */}
          <li className="hover:bg-white/10 transition rounded-lg">
            <a
              href="/dashboard/settings"
              className="flex items-center gap-2 p-2 text-sm text-white"
            >
          ...
          {/* Logout */}
          <li className="hover:bg-white/10 transition rounded-lg mt-1">
            <a
              href="/logout"
              className="flex items-center gap-2 p-2 text-sm text-Red"
            >
              <FiLogOut className="text-lg text-Red" />
              Log Out
            </a>
          </li>
        </ul>
